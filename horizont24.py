...
def show_fps():
    ...
    fps = f"Fps: {int(clock.get_fps())} {score=} {osc=}" # get the clocl'fps
...
def map_blit() -> list:
    ...
    def touchable_coins(lst, x,y):
        ...
        display.blit(coin_img, ((x * 16 - scroll[0]) + math.cos(osc)*5, (y * 16 - scroll[1]) + math.sin(osc)*5))
...
def move(rect,movement,tiles):
    ...
    for n, coin in enumerate(coin_rects):
